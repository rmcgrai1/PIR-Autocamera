...
#include <Arduino.h>
#include <C:\Program Files (x86)\Arduino\hardware\arduino\avr\libraries\SoftwareSerial\SoftwareSerial.h> // Allows for Serial Communication over pins 2 and 3
#include "math2.h"
#include "camvars.h"
#include "pir.h"
...
byte
  COMMAND_IR_RECEIVE[6]   = {0x81, 0x01, 0x06, 0x08, 0x02, 0xFF},                                 // Toggle IR Input
  ...
  COMMAND_CONFIG_MOTOR[8] = {0x81, 0x01, 0x7E, 0x01, 0x70, 0x00, 0x00, 0xFF},                     // Set Hard/Soft Stops
  COMMAND_PAN_STOP[9]     = {0x81, 0x01, 0x06, 0x01, 0x09, 0x09, 0x03, 0x03, 0xFF},               // Camera Pan Stop
  COMMAND_PAN[24]         = {0x81, 0x01, 0x06, 0x02, 0x00, 0x00,                                  // Pan Camera
  ...
  COMMAND_ZOOM[11]        = {0x81, 0x01, 0x7E, 0x01, 0x4A, 0x07, 0x00, 0x00, 0x00, 0x00, 0xFF},   // Zoom Camera
  COMMAND_FOCUS[6]        = {0x81, 0x01, 0x04, 0x38, 0x02, 0xFF},                                 // Enable Autofocusing
  
  COMMAND_IRIS[9]         = {0x81, 0x01, 0x04, 0x4B, 0x00, 0x00, 0x00, 0x00, 0xFF},               // Set Iris
  COMMAND_GAIN[9]         = {0x81, 0x01, 0x04, 0x4C, 0x00, 0x00, 0x00, 0x00, 0xFF},               // Set Gain
  COMMAND_BRIGHTNESS[9]   = {0x81, 0x01, 0x04, 0x4D, 0x00, 0x00, 0x00, 0x00, 0xFF},               // Set Brightness

  // PAGE 2
  COMMAND_BACKLIGHT[9]    = {0x81, 0x01, 0x04, 0x33, 0x00, 0xFF},                                 // Set Backlight
  COMMAND_APERTURE[9]     = {0x81, 0x01, 0x04, 0x42, 0x00, 0x00, 0x00, 0x00, 0xFF};               // Set Aperture
...
void setupCamera() {
  ...
  panPosition = toPanPosition = PIR_PAN[PIR_2],
  tiltPosition = PIR_TILT[PIR_2],
  zoomPosition = toZoomPosition = PIR_ZOOM[PIR_2];
...
void setCameraPosition(byte panSpeed, byte tiltSpeed, float panPosF, float tiltPosF) {
...
void setCameraAperture(float fraction) {
  ...
  convertFraction2Bytes(constrain(fraction, 0,1), 0, 31, COMMAND_APERTURE, 6);
