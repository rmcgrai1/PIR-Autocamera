...
// Array of All LEDS
  LED_PINS[] = {LED_PIN_11, LED_PIN_12, LED_PIN_13,
            LED_PIN_21, LED_PIN_22, LED_PIN_23, LED_PIN_24, LED_PIN_25, LED_PIN_26};
...
void writeLED(int led, int brightness) {
  ...
  brightness = (brightness == LOW) ? 0 : (brightness == HIGH) ? 255 : constrain(brightness, 0, 255);
...
void writeLEDPos(float angle, float fov) {
  ...
  for(int i = 3; i < 9; i++) {
    ...
    brightness = (int) constrain(255 - 40*fabs(calcAngleDiff(angle,ledDir)), 0,255);
