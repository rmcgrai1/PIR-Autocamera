...
const int 
  ...
  PIR_LIFETIME = 5*1000;
int
  inactiveTimer = 10*1000;
const byte
  PIR_PRESET[] = {PRESET_RIGHT, PRESET_MIDDLE, PRESET_LEFT};
float 
  PIR_TILT[] = {4,5,4},
  PIR_PAN[] = {CAMERA_FOV/2,0,-CAMERA_FOV/2},
  PIR_ZOOM[] = {CAMERA_ZOOM_MAX, CAMERA_ZOOM_MAX*.8, CAMERA_ZOOM_MAX};
...
class PIRChange {
  public:
    PIRChange(int pir) : pir(pir), time(timer) {};

    int getPIR() const    {return pir;}
...
void setupPIRs() {
  ...
  int k = 0, maxK = 0, advanceTime = calibrationTime/9, blinkTime = 30, d = 1;
...
bool analyzePIRS() {
  bool didUpdate = true;
  long  maxOn = maxi(pirTimeOn, 3),
        maxOff = mina(pirTimeOff, 3);
  ...
  if(pirTimeOn[maxOn] == -1) {
    if(timerPrevious-maxOff <= inactiveTimer && timer-maxOff > inactiveTimer) {
