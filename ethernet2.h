...
typeHandler&    addTypeHandler(char* type, boolean (*defaultHandler)(token_list_t*, char*, char*&), int aHandlerNum);
actionHandler&  addActionHandler(typeHandler& tHandler, char* action, boolean (*handler)(token_list_t*, char*, char*&));
boolean         handlerNull(token_list_t* jsonTokens, char* responseString, char* &curPos);

boolean         handlerLogGet(token_list_t* jsonTokens, char* responseString, char* &curPos);
boolean         handlerLogClear(token_list_t* jsonTokens, char* responseString, char* &curPos);

boolean         handlerCamGet(token_list_t* jsonTokens, char* responseString, char* &curPos);
boolean         handlerCamSetFocus(token_list_t* jsonTokens, char* responseString, char* &curPos);
...
void parseAction(String var, String val) {
  ...
  switch(varID) {
    case 1: Serial.println(val);  break;
    case 2: Serial.println(val);  break;
    case 3: Serial.println(val);  break;
    case 4: setFOV(value);  break;
  }
}
...
boolean handlerCamGet(token_list_t* jsonTokens, char *responseString, char* &curPos) {
  curPos += sprintf(curPos, "\"pan\":%d.%d,\"tilt\":%d.%d,\"zoom\":%d.%d,\"auto\":%d,\"pir1\":%d,\"pir2\":%d,\"pir3\":%d,\"fov\":%d.%d,\"mode\":%d,\"preset\":%d,",
    (int)panPosition,  decimal(panPosition, 3), 
    (int)tiltPosition, decimal(tiltPosition, 3),
    (int)zoomPosition, decimal(zoomPosition, 3),
    isAutoCam,
    checkPIR(PIR_1), checkPIR(PIR_2), checkPIR(PIR_3),
    (int)CAMERA_FOV,   decimal(CAMERA_FOV, 3),
    isPreset,
    camPreset);
...
boolean handlerLogGet(token_list_t* jsonTokens, char *responseString, char* &curPos) {
  unsigned long 
    startpos = parsel(json_get_value(jsonTokens, "pos")),
    cNum = 0,
    lineNum = 0,
    totalSize;
  ...
  if (webFile) {
    ...
    if(!webFile.seek(startpos))
      ...
      curPos += sprintf(curPos, "\"progress\":\"incomplete\",\"text\": \"");
      ...
      while(webFile.available()) {
        ...
        if(cc == '\"')
          ...
          if(lineNum >= LINE_MAX || cNum >= RESPONSE_LENGTH-HEADER_LENGTH)
            break;
        }
        else
      ...
      curPos += sprintf(curPos, "\",\"startPos\":%lu,\"pos\":%lu,\"total\":%lu,\"lineNum\":%lu,", 
        startpos,
        cNum,
        totalSize,
        lineNum);
...
typeHandler* tHandlers = (typeHandler*) malloc(TYPE_HANDLER_NUM * sizeof(typeHandler));
...
typeHandler& addTypeHandler(char* type, boolean (*defaultHandler)(token_list_t*, char*, char*&), int aHandlerNum) {
  ...
  tHandler.aHandlers = (actionHandler*) malloc(aHandlerNum * sizeof (actionHandler));
...
boolean runActionHandler(actionHandler& aHandler, token_list_t* jsonTokens, char* responseString, char* &curPos) {
  Serial.print("FOUND ACTION! ");
  Serial.println(aHandler.action);

  return aHandler.handler(jsonTokens, responseString, curPos);
}
boolean runTypeHandler(typeHandler& tHandler, token_list_t* jsonTokens, char* responseString, char* &curPos) {
  ...
  for(int i = 0; i < tHandler.aHandlerNum; i++) {
    ...
    if(!strcmp(aHandler.action, action))
      return runActionHandler(aHandler, jsonTokens, responseString, curPos);
  }
  
  return tHandler.defaultHandler(jsonTokens, responseString, curPos);
}
boolean runHandler(token_list_t* jsonTokens, char* responseString, char* &curPos) {
      ...
      if(client.connected()) {
        ...
        while(client.connected()) {        
          if(client.available()) {   // client data available to read
              ...
              char responseString[RESPONSE_LENGTH] = "",
                *curPos = responseString;
              ...
              if(true) {
                ...
                char jsonStr[256] = "";
                token_list_t *jsonTokens = create_token_list(10*2 + 1);
                ...
                while(i < len) {
                  ...
                  while(eatChar(clientText,c,i,len) != '=' && c != -1)
                    strcat(jsonStr, c);
                  strcat(jsonStr, ":");
                  while(eatChar(clientText,c,i,len) != '&' && c != -1)
                    strcat(jsonStr, c);
                ...
                curPos += sprintf(curPos, "{\"type\":\"%s\",\"action\":\"%s\",",
                  json_get_value(jsonTokens, "type"),
                  json_get_value(jsonTokens, "action"));
                
                success = runHandler(jsonTokens, responseString, curPos);

                if(!success) {
                  if(type == "log") {
                    if(action == "deleteLines") {
                      success = true;
                      deleteUpTo = parsei(json_get_value(jsonTokens, "upto"));
                    }
                  }
                  else if(type == "cam") {
                    success = true;
                    ...
                    if(action == "move-ptz") {
                      String txt = "Client requested PTZ camera movement.";
                      logData(txt);
      
                      toPanPosition = parsef(json_get_value(jsonTokens, "pan"));
                      tiltPosition = parsef(json_get_value(jsonTokens, "tilt"));
                      toZoomPosition = parsef(json_get_value(jsonTokens, "zoom"));
                      ...
                      clearCameraBuffer();
                      setCameraPreset(parsei(json_get_value(jsonTokens, "preset")));
                    }
                    else if(action == "set-aperture")
                      setCameraAperture(parsef(json_get_value(jsonTokens, "value")));
                    else if(action == "set-backlight")
                      setCameraBacklight(parsei(json_get_value(jsonTokens, "value")) ? true:false);
                    else
                      success = false;
  
                  }
                  else if(type == "pir") {
                    int id = parsei(json_get_value(jsonTokens, "id"));
                    if(action == "on") {
                      success = true;
                      activatePIR(id,true);
                    }
                    ...
                    if(action == "on") {
                      ...
                      logData("Automatic control set to \"off\".");
                    }
                  }
                  else if(type == "mode") {
                    if(action == "preset") {
                    ...
                    byte preset;
                    String presetStr = json_get_value(jsonTokens, "preset");
                ...
                curPos += sprintf(curPos, "\"result\":\"%s\"}", success?"success":"error");
